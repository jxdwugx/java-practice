...
public class ComponentContext {
     ...
     * 销毁方法map
     */
    private Map<String, Method> destroyMethodMap = new HashMap<>();
    ...
    protected void instantiateComponents() {
        ...
        System.out.println("获取到的bean" + this.getComponent("bean/DBConnectionManager"));;
    }
    ...
    protected void initializeComponents() {
        componentsMap.forEach((name, component) -> {
            ...
            // 销毁阶段
            processPreDestroy(name, componentClass);
        });
    }
    ...
    private void processPreDestroy(String name, Class<?> componentClass) {
        Stream.of(componentClass.getDeclaredMethods())
                .filter(field -> {
                    int mods = field.getModifiers();
                    return !Modifier.isStatic(mods) &&
                            field.isAnnotationPresent(PreDestroy.class);
                }).forEach(method -> destroyMethodMap.put(name, method));
    }
    ...
    public void destroy() throws RuntimeException {
        destroyMethodMap.forEach((name, method) -> {
